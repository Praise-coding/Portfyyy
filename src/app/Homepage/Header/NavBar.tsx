...
import MenuPopUp from "@/app/Homepage/Header/MenuPopUp";
import NavBarScroll from "@/app/Homepage/Header/NavBarScroll";
...
function NavBar() {
    ...
    return (
        <>
            <nav className={"flex px-[20px]  items-center   justify-center py-[16px]"}>
                <AnimateCon once={false} hidden={{opacity: 0}} visible={{opacity: 100, transition: {duration: 0.7}}}
                            htmlElement={"div"}
                            className={"max-w-[1170px] w-full justify-between flex items-center"}>
                    ...
                    <div onClick={() => setDisplay((prevState) => !prevState)}
                         className="p-[16px] border relative z-[99900] border-white rounded-[6px] flex md:hidden">
            ...
            <NavBarScroll setDisplay={setDisplay}/>
            <MenuPopUp display={display}/>
        </>
